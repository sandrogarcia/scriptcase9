...
function nm_window_image_new(str_folder, str_form, str_field, str_cback, int_index, isChild)
{
    str_index = (null == int_index || '' == int_index) ? '' : '&index=' + int_index;
	if ('' == str_index && document.forms[str_form].elements[str_field] != undefined)
	{
		str_val = document.forms[str_form].elements[str_field].value;
	}
	else if( document.forms[str_form].elements[str_field + "[" + int_index + "]"] != undefined)
	{
		str_val = document.forms[str_form].elements[str_field + "[" + int_index + "]"].value;
	}
	else
    {
        str_val = $('input[name='+ str_field +']').val();
    }
    if (typeof str_cback === 'undefined' || null == str_cback) { str_cback = ''; }
    if (typeof isChild === 'undefined' || null == isChild) { isChild = true; }

    blockUiAppForImage(str_folder, str_form, str_field, str_val, str_cback, str_index, isChild);
}
...
function nm_window_image_toolbar(id_field, str_cback)
{
    ...
    blockUiAppForImage("ico", "toolbar", id_field, str_val, str_cback, str_index, isChild);
}
...
function nm_window_manual(str_url)
{
 if(str_url != '#')
 {
   obj_win = window.open(str_url, "nmWinWebhelpV91_" + nm_win_name).focus();
   setTimeout(function(){  obj_win.focus(); },300);
...
function nm_window_upload(str_mod, not_exib, str_refresh)
{
	...
	nm_window_inside('upload', nm_url_iface + "index.html");
...
function nm_window_inside(str_mod, url, height, width)
{
	...
	$('#id_window'+str_mod+' > iframe').on('load', function(){
