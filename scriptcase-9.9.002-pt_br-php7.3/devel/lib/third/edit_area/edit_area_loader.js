...
EditAreaLoader.prototype ={
	...
	start : function(id){
		...
		// ----------------------------------scriptcase-------------------------------------------------------------------------------------------------
		select_macro_complete = '';
		if (area["settings"]["macro_complete"] != '')
		{
			select_macro_complete = "<select id='sel_macro_complete' ondblclick='InsertMacroTextarea();' size='10'>";
			arr_macro = area["settings"]["macro_complete"].split('__|macro|__');
			for(i_macro = 0; i_macro < arr_macro.length; i_macro++)
			{
				macro = arr_macro[i_macro].split('__|=|__');
				select_macro_complete += "<option value=\""+ macro[1] +"\" >"+ macro[0] +"</option>";
			}
			select_macro_complete += "</select>";
			select_macro_complete += "<input type='hidden' id='n_row_macro_complete' />";
			select_macro_complete += "<input type='hidden' id='n_col_macro_complete' />";
			select_macro_complete += "<input type='hidden' id='n_chr_macro_complete' />";
			select_macro_complete += "<input type='hidden' id='s_dig_macro_complete' />";
			select_macro_complete += "<input type='hidden' id='n_lft_macro_complete' />";
			select_macro_complete += "<input type='hidden' id='n_top_macro_complete' />";
		}
		if (area["settings"]["div_editor_style"])
		{
			div_editor_style = area["settings"]["div_editor_style"];
		}
		else
		{
			div_editor_style = "";
		...
		// ----------------------------------scriptcase-------------------------------------------------------------------------------------------------
		template= template.replace("[__MACRO_COMPLETE__]", select_macro_complete);
		template= template.replace("[__DIV_EDITOR_STYLE__]", div_editor_style);		
		template= template.replace("[__TOOLBAR__]", "<span id='toolbar_extra'></span>" + html_toolbar_content);			
		template= template.replace("[__TOOLBAR1_STYLE__]", (area["settings"]["tab_toolbar"].length <= 1 ? 'display:none' : ''));
		template= template.replace("[__TOOLBAR2_STYLE__]", (area["settings"]["tab_toolbar"].length <= 1 ? 'display:none' : ''));		
		// ----------------------------------scriptcase-------------------------------------------------------------------------------------------------
