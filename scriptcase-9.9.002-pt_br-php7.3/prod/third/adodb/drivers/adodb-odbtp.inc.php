...
class ADODB_odbtp extends ADOConnection{
	...
	function CreateSequence($seqname='adodbseq',$startID=1)
	{
		...
		$startID -= 1;
		return $this->Execute("insert into adodb_seq values('$seqname',$startID)");
	}
	...
	function GenID($seqname='adodbseq',$startID=1)
	{
		...
		while (--$MAXLOOPS>=0) {
			...
			if ($num === false) {
				...
				$startID -= 1;
				$num = '0';
				$ok = $this->Execute("insert into adodb_seq values('$seqname',$startID)");
				if (!$ok) return false;
			}
			$ok = $this->Execute("update adodb_seq set seq_value=seq_value+1 where seq_name='$seqname'");
			if($ok) {
				$num += 1;
				$this->genID = $num;
				return $num;
