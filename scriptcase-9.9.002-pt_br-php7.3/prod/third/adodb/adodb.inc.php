...
/*------*/
		 ...
		 */
		$ADODB_vers = 'v4.991 16 Oct 2008 (c) 2000-2008 John Lim (jlim#natsoft.com). All rights reserved. Released BSD & LGPL.';
...
*/
	...
	var $bol_sc_debug       = false; // debug do scriptcase
	var $bol_sc_debug_level = 0;    // nivel debug do scriptcase
	...
	*/
	...
	function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false, $arrExtraArgs=array(), $charset="")
	{
		...
		if ($forceNew) {
			if($this->bol_sc_debug)
			{
				...
				$this->sc_start_debug("Connect force new", "", $time_start);
			}
			if ($rez=$this->_nconnect($this->host, $this->user, $this->password, $this->database, $arrExtraArgs, $charset)) return true;
			if($this->bol_sc_debug)
			{
				$this->sc_finalize_debug($time_start);
			}
		} else {
			if($this->bol_sc_debug)
			{
				$time_start = $this->microtime_float();
				$this->sc_start_debug("Connect", "", $time_start);
			}
			if ($rez=$this->_connect($this->host, $this->user, $this->password, $this->database, $arrExtraArgs, $charset)) return true;
			if($this->bol_sc_debug)
			{
				$this->sc_finalize_debug($time_start);
			}
	...
	function _nconnect($argHostname, $argUsername, $argPassword, $argDatabaseName, $arrExtraArgs=array(), $charset="")
	{
		...
		{
			$time_start = $this->microtime_float();
			$this->sc_start_debug("NConnect", "", $time_start);
		}
		$retorno = $this->_connect($argHostname, $argUsername, $argPassword, $argDatabaseName, $arrExtraArgs, $charset);
		if($this->bol_sc_debug)
		{
			$this->sc_finalize_debug($time_start);
		}
	 ...
	 * @return true or false
	 */
	function NConnect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $arrExtraArgs=array(), $charset="")
	{
		return $this->Connect($argHostname, $argUsername, $argPassword, $argDatabaseName, true, $arrExtraArgs, $charset);
	}
	 ...
	 */
	function PConnect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $arrExtraArgs = array(), $charset = "")
	{
		if (defined('ADODB_NEVER_PERSIST'))
			return $this->Connect($argHostname,$argUsername,$argPassword,$argDatabaseName, false, $arrExtraArgs, $charset);
		...
		{
			$time_start = $this->microtime_float();
			$this->sc_start_debug("PConnect", "", $time_start);
		}

		if ($rez = $this->_pconnect($this->host, $this->user, $this->password, $this->database, $arrExtraArgs, $charset)) return true;
	...
	*/
	...
	function sc_start_debug($comando, $sql, $start)
	{
		$ident = str_repeat(" ", $this->bol_sc_debug_level * 8);

		$str_file  = $ident . "Comando ". $comando ."\r\n";
		$str_file .= $ident . "Provider: ". $this->dataProvider ."\r\n";
		$str_file .= $ident . "DBMS: ". $this->databaseType ."\r\n";
		$str_file .= $ident . "SERVER: ". $this->host ."\r\n";
		$str_file .= $ident . "DB: ". $this->database ."\r\n";
		$str_file .= $ident . "SQL:\r\n";
		$str_file .= $ident . $sql . "\r\n";

		$arr_debug = debug_backtrace();
		$str_file .= $ident . "Debug: \r\n";
		if(is_array($arr_debug))
		{
			foreach($arr_debug as $arr)
			{
				if(!isset($arr['line']))
				{
					$arr['line'] = "";
				...
				$str_file .= $ident . " " . $arr['file'] . "(". $arr['function'] . " - " . $arr['line'] .")\r\n";
			}
		}

		file_put_contents(ADODB_DIR . '/log/sql_logs.txt', $str_file, FILE_APPEND);

		$this->bol_sc_debug_level++;
	}

	function sc_finalize_debug($start)
	{
		$this->bol_sc_debug_level--;
		...
		$total = $end - $start;
		$str_file  = $ident . "Execu��o: ". $total ." segundos\r\n";
		$str_file .= $ident . "\r\n-------------------------------------------------------------------------\r\n\r\n";
	...
	function Execute($sql,$inputarr=false)
	{
		...
		{
			$time_start = $this->microtime_float();
			$this->sc_start_debug("Execute", $sql, $time_start);
		}
		...
		if ($inputarr) {
			...
			if (!is_array($sql) && !$this->_bindInputArray) {
				...
				foreach($inputarr as $arr) {
					...
					if (isset($sqlarr[$i])) {
						$sql .= $sqlarr[$i];
						if ($i+1 != sizeof($sqlarr)) ADOConnection::outp( "Input Array does not match ?: ".htmlspecialchars($sql, ENT_COMPAT, "ISO-8859-1"));
					} else if ($i != sizeof($sqlarr))
						ADOConnection::outp( "Input array does not match ?: ".htmlspecialchars($sql, ENT_COMPAT, "ISO-8859-1"));
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	function SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
		...
		{
			$time_start = $this->microtime_float();
			$this->sc_start_debug("SelectLimit", $sql . "\r\nLIMIT - Rows: ". $nrows . " - Offset: " . $offset, $time_start);
		}

		if ($this->hasTop && $nrows > 0) {
		// suggested by Reinhard Balling. Access requires top after distinct
		 // Informix requires first before distinct - F Riosa
			$ismssql = ((strpos($this->databaseType,'mssql') !== false) || ($this->databaseType=='pdo' && isset($this->dsnType) && $this->dsnType=='sqlsrv'));
	...
	global $ADODB_CACHE;
		...
		// ok, set cached object found
			...
			if ($this->debug){
				...
				if ($inBrowser) $s = '<i>'.htmlspecialchars($s, ENT_COMPAT, "ISO-8859-1").'</i>';
...
*/
 ...
 /**
   ...
   * $upper  0 = lowercase, 1 = uppercase, 2 = whatever is returned by FetchField
   */
	function GetRowAssocKey($upper=1)
	{
		$record = array();
