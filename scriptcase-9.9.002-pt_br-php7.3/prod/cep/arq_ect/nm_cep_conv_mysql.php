...
//---  Convers�o dos arquivos do correio
//
    set_time_limit(0);
...
*/
    ...
    {
        die("Erro ao estabelecer uma conex�o com o banco de dados = " . $Db->ErrorMsg());
        exit;
    }
    ...
    switch($_GET['step'])
    {
        ...
        case 5:
            ...
            $Db->Close();
            echo "<br>** Convers�o Concluida com Sucesso **";
            echo "\r\n</body></html>";
            exit;
        break;
    }
    ...
    while (!$rs_fx_est->EOF)
    {
        ...
        if ($mudou) {
            if (!empty($estado)) {
                ...
                $dados_saida = implode("@nm@", $dados);
                fwrite($arq_saida, $dados_saida . "@nm@\r\n");
                /* Arquivos n�o mais utilizados. Gravados apenas para n�o dar erro nas rotinas de CEP da produ��o */
...
function conv_fx_localidades()
{
   ...
   where cepbr_cidade.cep is null OR cepbr_cidade.cep = ''
...
function conv_logradouros()
{
   ...
   order by cepbr_cidade.uf, cepbr_cidade.cidade, cepbr_endereco.logradouro";
    ...
    while (!$rs_logr->EOF)
    {
        ...
        $dados[1] = "";                         // Titulo da rua (n�o existe mais)
        ...
        if ($dados[0] == "DF")
        {
            ...
            if (substr($tipo, 0, 4) == "�rea" && (strpos($dados[2], "�rea") !== false || substr($dados[2], 0, 1) == "A"))
            {
               $limpa_tipo = true;
            }
